...
TOP10 = [
    ...
    ("BIOTIN",              "C1C2C(C(S1)CCCCC(=O)O)NC(=O)N2", -6.76),
    ("GABAPENTIN_ENACARBIL","CC(OC(=O)NCC1(CC(=O)O)CCCCC1)OC(=O)C(C)C", -6.63),
    ("RIBOFLAVIN",          "CC1=CC2=C(C=C1C)N(C[C@@H]([C@@H]([C@@H](CO)O)O)O)C3=NC(=O)NC(=O)C3=N2", -0.01),
]
RUN_LIST = [n for n,_,_ in TOP10]  # All 6 at 50ns
...
def run_md(name, smiles, prod_ns):
    ...
    try:
        ...
        # ── 2. Get starting coordinates (Vina pose or RDKit fallback) ──
        pdbqt = f"{DOCKING_DIR}/{name}.pdbqt"
        vina_pos = extract_vina_pose(pdbqt) if os.path.exists(pdbqt) else []
        if not vina_pos:
            # Fallback: use RDKit conformer
            log.info(f"  No Vina pose, using RDKit conformer")
            mol = Chem.MolFromSmiles(smiles)
            mol = Chem.AddHs(mol)
            AllChem.EmbedMolecule(mol, randomSeed=42)
            AllChem.MMFFOptimizeMolecule(mol, maxIters=500)
            conf = mol.GetConformer()
            vina_pos = [list(conf.GetAtomPosition(i)) for i in range(mol.GetNumAtoms())]
        log.info(f"  Starting pose: {len(vina_pos)} atoms")
