#!/usr/bin/env python3
"""Dock 788 filtered undocked ChEMBL FDA-approved drugs."""
...
for _, r in df.iterrows():
    candidates.append({"name": str(r["name"]), "smiles": str(r["smiles"])})
log.info(f"Loaded {len(candidates)} FDA drugs to dock")
...
for i, c in enumerate(pending):
    ...
    try:
        if not os.path.exists(pdbqt) or os.path.getsize(pdbqt) < 100:
            # Use subprocess for all OpenBabel ops (avoids pybel hangs on Windows)
            sdf = os.path.join(DOCK_DIR, f"{safe}_fda.sdf")
            result_3d = subprocess.run(
                ["obabel", f"-:{c['smiles']}", "-osdf", "-O", sdf, "--gen3d", "--minimize", "--steps", "500", "--ff", "MMFF94"],
                capture_output=True, text=True, timeout=60
            )
            if result_3d.returncode != 0 or not os.path.exists(sdf) or os.path.getsize(sdf) < 50:
