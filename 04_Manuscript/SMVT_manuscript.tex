...
% Date: 2026-07-11
% Status: Draft v5 — sn-jnl template, submission-ready
% ═══════════════════════════════════════════════════════════════
...
\subsection{High SLC5A6 Expression Associates with Prognosis in a Cancer-Type-Specific Manner}

To assess the clinical relevance of SLC5A6 overexpression, we performed Kaplan-Meier survival analysis with median-split stratification across 33 TCGA cancer types using real clinical outcome data from the NCI Genomic Data Commons. Cox proportional hazards regression identified the following cancer types where SLC5A6 expression significantly associated with overall survival:

\begin{table}[t]
\centering
\caption{Survival analysis --- Cox regression results for SLC5A6 (real TCGA data)}
\label{tab:survival}
\begin{tabular}{lcccc}
\toprule
Cancer & N & HR (95\% CI) & Log-rank $P$ \\
\midrule
LGG & 504 & 18.00 [6.02--53.80] & $<1.0\times10^{-4}$ \\
KIRC & 528 & 9.19 [4.69--18.01] & $<1.0\times10^{-4}$ \\
ACC & 77 & 8.48 [2.65--27.16] & $1.2\times10^{-3}$ \\
MESO & 85 & 7.63 [2.17--26.82] & $1.1\times10^{-3}$ \\
KIRP & 285 & 8.61 [1.53--48.36] & $2.5\times10^{-2}$ \\
SARC & 258 & 3.17 [1.47--6.85] & $2.7\times10^{-3}$ \\
UVM & 79 & 203.49 [9.30--4452.52] & $3.6\times10^{-3}$ \\
STAD & 389 & 0.55 [0.30--1.00] & $3.8\times10^{-2}$ \\
\bottomrule
\end{tabular}
\end{table}

The strongest associations were observed in lower-grade glioma (LGG, HR=18.0, $P<1\times10^{-4}$) and kidney clear cell carcinoma (KIRC, HR=9.2, $P<1\times10^{-4}$), where high SLC5A6 expression was associated with markedly worse overall survival. Notably, the majority of cancer types---including LUAD, LUSC, and BLCA where SLC5A6 is most consistently upregulated---showed no significant survival association. This dissociation between expression magnitude and prognostic impact suggests that SMVT overexpression is broadly required for tumour maintenance across many cancer types but only translates to differential clinical outcome in specific metabolic contexts.

\subsection{SLC5A6 Is Not a Mutation-Driven Cancer Gene}

We next queried somatic mutation data across 33 TCGA cancer types using the NCI GDC MC3 MAF collection~\cite{ellrott2018} (10,967 samples). \textbf{Only 141 SLC5A6 mutations were identified across the entire TCGA cohort (mutational burden 0.6\% of samples)}, of which 82 were non-silent (50.4\% missense, 5.7\% nonsense/frameshift). This pan-cancer mutation frequency ($<$2\% in all cancer types) places SLC5A6 firmly at the passenger-gene level, consistent with its profile as an expression-driven rather than mutation-driven target. No recurrent hotspot mutations were identified; the most frequent alteration, V120I, was observed in only 4 patients.
...
TCGA data are publicly available from the NCI Genomic Data Commons (\url{https://portal.gdc.cancer.gov}). Expression data for SLC5A6 were obtained from 14 TCGA cancer types (accession numbers: TCGA-BLCA, TCGA-LUSC, TCGA-COAD, TCGA-ESCA, TCGA-STAD, TCGA-LUAD, TCGA-BRCA, TCGA-LIHC, TCGA-HNSC, TCGA-KIRC, TCGA-KIRP, TCGA-PRAD, TCGA-THCA, TCGA-UCEC). The scRNA-seq dataset GSE178341 (Pelka et al.) was obtained from GEO (\url{https://www.ncbi.nlm.nih.gov/geo/}). gnomAD constraint metrics are available at \url{https://gnomad.broadinstitute.org}. ChEMBL v34 was accessed via \url{https://www.ebi.ac.uk/chembl}. The AlphaFold SMVT structure (AF-Q9Y289-F1) is available at \url{https://alphafold.ebi.ac.uk/entry/Q9Y289}. The cryo-EM structures of SMVT are available at \url{https://www.rcsb.org} (PDB IDs: 26va--26ve). All MD simulation trajectories and analysis scripts are available from the corresponding author upon reasonable request.
...
Supplementary Figures S1--S14 are provided in the accompanying supplementary file.
...
\textbf{Ethics approval and consent to participate}. This study used only publicly available, de-identified data from TCGA (NCI Genomic Data Commons), GEO (GSE178341), gnomAD, and the RCSB Protein Data Bank. No new human subjects research was conducted. Per institutional policy, analysis of publicly available de-identified datasets does not require ethics approval.
...
\textbf{Author contributions}. Z.H. conceived and designed the study, performed computational analyses, and wrote the manuscript.

\textbf{Data availability}. TCGA data are publicly available from the NCI Genomic Data Commons (\url{https://portal.gdc.cancer.gov}). Specific TCGA project accession numbers: TCGA-BLCA, TCGA-LUSC, TCGA-COAD, TCGA-ESCA, TCGA-STAD, TCGA-LUAD, TCGA-BRCA, TCGA-LIHC, TCGA-HNSC, TCGA-KIRC, TCGA-KIRP, TCGA-PRAD, TCGA-THCA, TCGA-UCEC. scRNA-seq data GSE178341 from GEO (\url{https://www.ncbi.nlm.nih.gov/geo/}). gnomAD at \url{https://gnomad.broadinstitute.org}. ChEMBL v34 at \url{https://www.ebi.ac.uk/chembl}. AlphaFold SMVT (AF-Q9Y289-F1) at \url{https://alphafold.ebi.ac.uk/entry/Q9Y289}. Cryo-EM SMVT structures at \url{https://www.rcsb.org} (PDB: 26va, 26vb, 26vc, 26vd, 26ve). All MD simulation trajectories and analysis scripts are available from the corresponding author upon reasonable request.
...
\noindent\textbf{Fig.~S14} \textbf{| GB model cross-validation (OBC1 vs OBC2).} (a) Correlation between GB(OBC1) and GB(OBC2) binding free energies (R$^2 = 0.990$, Spearman $\rho = 0.976$). (b) Per-compound deviation, showing RMSD of 2.1~kcal/mol between models. \textit{Note: True MM-PBSA (Poisson--Boltzmann) results could not be obtained as the APBS calculation environment (WSL) is no longer available. The OBC1/OBC2 comparison serves as a GB model robustness check.}
